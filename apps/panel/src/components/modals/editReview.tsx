...
interface IEditReviewProps extends IReview {
    onDone: () => void
}
export default function EditReview(props: IEditReviewProps) {
    const { setModal } = useModal()
    const [reviewer_name, setReviewerName] = useState(props.reviewer_name)
    const [reviewer_company, setReviewerCompany] = useState(props.reviewer_company)
    const [reviewer_image, setReviewerImage] = useState<string | null>(props.reviewer_image)
    const [review_message, setReviewerMessage] = useState(props.review_message)
    ...
    const handleEditReview = async () => {
        ...
        try {
            setLoading(true)
            const res = await axios.put(`/reviews/${props.id}`, { reviewer_company, reviewer_image, reviewer_name, review_message }, {
                headers: {
    ...
    return (
        <div className="bg-white p-8 rounded-md w-[500px]  flex flex-col gap-4">
            <SectionTitle title="Edit review" description="Make changes and hit save" />
            ...
            <div className="flex items-center justify-between gap-6">
                ...
                <Button variant={'primary'} className="bg-brand-lightblue  rounded-md" loading={loading} onClick={handleEditReview}>Save</Button>
            </div>
        </div>
    );
}